...
fn split_pane_demo_flow_opens_file_in_other_window() -> Result<()> {
    ...
    rile.assert_status_contains("ACTIVE split_left.txt")?;
    ...
    rile.assert_screen_contains("left 000 | alpha")?;
    rile.assert_screen_contains("inactive split_left.txt")?;
    rile.assert_status_contains("ACTIVE split_right.txt Ln 001 Col 00")?;

    rile.send("C-n", keys::control('n'))?;
    rile.assert_status_contains("ACTIVE split_right.txt Ln 002 Col 00")?;

    rile.send("C-n", keys::control('n'))?;
    rile.assert_status_contains("ACTIVE split_right.txt Ln 003 Col 00")?;

...
Window splitting stores per-window cursor state and scrolls automatically to keep point visible, including Emacs-style horizontal recentering on clipped long lines. Normal long-line redraws project a bounded source prefix into the visible viewport; pathological zero-width runs are truncated with the normal edge marker. Side-by-side splits reserve a visible separator column. Empty rows are left blank rather than filled with marker characters.
